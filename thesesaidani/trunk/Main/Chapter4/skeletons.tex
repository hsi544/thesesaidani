...
\begin{figure}[!htb]
	\centering
  \includegraphics[width= 0.8\columnwidth]{Chapter4/figures/skell-pardo}
...
\begin{figure}[htb]
	\centering
  \includegraphics[width= \columnwidth]{Chapter4/figures/fullchain_pardo8}
	\caption{Squelette de la version complètement chaînée}
	\label{fullchain_pardo8}
\end{figure}
\begin{figure}[htb]
	\centering
  \includegraphics[width= \columnwidth]{Chapter4/figures/halfchain_pardo4}
	\caption{Squelette de la versions chaînée à moitié}
	\label{halfchain_pardo4}
\end{figure}
\begin{figure}[htb]
	\centering
  \includegraphics[width= \columnwidth]{Chapter4/figures/nochain_pardo2}
	\caption{Squelette de la version non chaînée}
	\label{nochain_pardo2}
\end{figure}

Cette application est plus complexe que celles vues précédemment car elle comporte plusieurs opérateurs, à la fois point-à-point et noyaux de convolution. Ce benchmark a pour but de prouver que \texttt{SKELL BE} est également adapté pour le traitement d'images. Plusieurs schémas de parallélisation sont possibles pour cet algorithme, comme vu dans le chapitre 2. Nous avons choisi ici trois versions de déploiement :  une version complètement chaînée (Fig. \ref{fullchain_pardo8}) où tous les opérateurs sont regroupés au sein d'un seul et même SPE et dupliqués 8 fois; la versions chaînée à moitié (Fig. \ref{halfchain_pardo4}), où les opérateurs sont chaînés deux à deux sur un SPE, un \emph{pipeline} est ensuite formé entre deux SPEs ce qui permet de dupliquer 4 fois; Et enfin une version où chaque opérateur occupe un SPE (Fig. \ref{nochain_pardo2}),  ce qui permet de dupliquer 2 fois. Le listing \ref{harrisskell} représente les \emph{kernels} SPE des différentes versions.  Le chaînage y est représenté par une virgule (\textbf{,}) et le \emph{pipeline} par un opérateur \textbf{\textbar}.
\newpage
%===============================Listing transfill =======================================
\lstset{ %
language=C,                % choose the language of the code
...
\end{table}
\subsection{Impact sur la taille de l'exécutable}
...
\hline
\end{tabular}
\caption{Impact sur la taille de l'exécutable}\label{codesize}
\end{table}
...
\subsection{Gain en expressivité}
Un des principaux apports de la programmation par squelettes algorithmiques et le gain en expressivité. En effet, tous les aspects liés à la création des \emph{threads}, à la distribution des données et à la synchronisation ne sont pas exposés au programmeur. La représentation du schéma de calcul sous forme de squelette permet la génération du code de calcul, de transfert et de synchronisation correspondant. On peut ainsi constater à partir des listings \ref{harris::spmd::ppe} et \ref{harris::spmd::spe}, le gain considérable en expréssivité. On peut comparer ce code source avec celui de la ligne 3 dans le listing \ref{harrisskell}.
%===============================Listing HARRIS::SPMD::PPE =======================================
\lstset{ %
language=C,                % choose the language of the code
basicstyle=\footnotesize,       % the size of the fonts that are used for the code
backgroundcolor=\color{light-gray},  % choose the background color. You must add \usepackage{color}
...
breakatwhitespace=false,	% sets if automatic breaks should only happen at whitespace
escapeinside={\%*}{*)}  ,        % if you want to add a comment within your code
caption = Exemple d'implantation de la version SPMD en \emph{pthreads} : code du PPE,
label = harris::spmd::ppe
}
\lstinputlisting{Chapter4/Code/harris_spmd_ppe.c}
%===============================Listing HARRIS::SPMD::SPE =======================================
\lstset{ %
language=C,                % choose the language of the code
basicstyle=\footnotesize,       % the size of the fonts that are used for the code
backgroundcolor=\color{light-gray},  % choose the background color. You must add \usepackage{color}
...
breakatwhitespace=false,	% sets if automatic breaks should only happen at whitespace
escapeinside={\%*}{*)}  ,        % if you want to add a comment within your code
caption = Exemple d'implantation de la version SPMD en \emph{pthreads} : code du SPE,
label = harris::spmd::spe
}
\lstinputlisting{Chapter4/Code/harris_spmd_spe.c}
