...
\subsubsection{Chainage d'opérateurs par paires : Fig. \ref{harris-hchain}}
\begin{figure}[!htb]
	\centering
...
\subsubsection{Chainage et fusion d'opérateurs par paires : Fig. \ref{harris-hchainhpipe}}
\begin{figure}[!htb]
	\centering
...
\begin{figure}[htb]
  ...
  \includegraphics[width= 0.8\columnwidth]{Chapter5/figures/BenchCompareModels}
	...
	\label{comparemodels}
\end{figure}
La figure \ref{comparemodels} donne une comparaison entre les différents modèles de déploiement de l'algorithme de Harris sur le processeur Cell décrits précédemment. Ce que l'on peut observer en premier lieu c'est que le schéma \emph{pipeline} donne les plus mauvaises performances ce qui était attendu : cette version n'exploite pas entièrement le parallélisme de données offert par l'architecture du Cell. Les autres résultats correspondent également à nos attentes : 
\begin{itemize}
...
\item Les versions dans lesquelles les DMA inter-SPE sont utilisés possèdent de bonnes performance : à titre d'exemple la versions sans composition de fonction et en regroupant deux opérateurs par SPE est plus rapide que la versions data parallel.
