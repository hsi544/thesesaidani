...
\indent Au vu de la nature de la hiérarchie mémoire du processeur Cell, le \emph{tiling} est une obligation. D'une part, il n'existe pas de mémoire cache pour gérer les mémoires privée des SPEs. D'autre part l'espace de stockage est limité dans les mémoires locales des SPEs à 256 Ko. Ceci rend le découpage des données en tuiles pouvant tenir dans le cache obligatoire. L'unité de données atomique devient alors la tuile qui représente le morceau de donné le plus petit qui va être traité par le code du SPE.
