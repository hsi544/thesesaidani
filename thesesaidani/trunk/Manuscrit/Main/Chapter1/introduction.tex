...
\subsection{Le Modèle \emph{Message Passing}}
...
\begin{itemize}
\item \textbf{Chapitre 1, Architecture du processeur Cell et environnement de développement}. Nous présentons dans ce chapitre une vue d'ensemble de l'architecture principale étudiée lors de nos travaux : le Cell Broadband Engine. Les principaux composants y sont décrits ainsi que les différents niveaux de parallélisme de l'architecture. L'environnement de développement de base fourni par IBM est également décrits avec une vue globale de l'API et des contraintes de programmation spécifiques à l'architetcure du Cell. 
\item \textbf{Chapitre 2, Outils de programmation tierces}. Un inventaire non-exaustif des outils de programmation développés à la fois par les laboratoires de recherches, ainsi que des outils industriels sont présentés. Les modèles de programmation associés aux outils y sont présentés. Le but de cette analyse est également de comparer les approches pour en extraire les avantages et les incovénients pour l'aide au portage de code sur l'architecture du processeur Cell.
