...
\subsubsection{Méthode et Plateforme de Mesure}
La plate-forme sur laquelle a été menée l'evaluation des performance est un Blade QS20 d'IBM cadencé à 3.2 GHz. Cette lame contient deux processeurs Cell, chacun relié à une mémoire de 512 MB, ce qui donne au total 1 GB de \emph{RAM} disponible. Le timer utilisé pour mesure la performance est appelé \emph{timebase} et il est cadencé à 14.318 $Mhz$. L'OS installé est un Linux, distribution Fedora 7. Le developpement à été réalisé grâce au \emph{Cell SDK 3.0} et les compilateurs utilisés sont \emph{ppu-gcc} et {spu-gcc}, ce qui implique une compilation du type \emph{dual-source} un pour le SPE et un pour le PPE.\\
\indent La méthodologie de mesure adoptée est en accord avec les système de vision par ordinateur. Nous avons simulé le cas d'un flux d'images continu, et avons mesure le temps d'exécution moyen pour une image. Ceci permet de négliger l'\emph{overhead} induit par la création et la synchronisation des \emph{threads}. L'interface utilisateur permet de faire varier plusieurs paramètres, notamment la taille de l'image, la taille de la tuile et le nombre de SPEs.
...
\begin{itemize}
\item Nos techniques d'optimisations des accès mémoires améliorent sensiblement la performance globale car la versions pla plus rapide est celle ou les fonctions sont composées deux par deux  et ou tous les opérateurs sont exécutés sur le même SPE (remplacement des DMA par des \texttt{LOAD/STORE} ).
\item Les versions ou les DMA inter-SPE sont utilisés possèdent de bonnes performance : à titre d'exemple la versions sans composition de fonction et en regroupant deux opérateurs par SPE est plus rapide que la versions .
...
Lorsque l'on mesure les performance d'une machine parallèle on ne peut se contenter de mesurer les performances temporelles brutes. Il est également intéressant de s'attarder sur d'autres paramètres qui permettent de comparer deux architectures parallèles ou alors d'isoler un goulot d'étranglement qui limite la performance. Ce que l'on veut évaluer est le passage à l'échelle ou \emph{scalability} qui permet de mesurer sur un système à plusieurs processeurs, le rendement de $N$ unités par rapport à un seul. Les métriques les plus connues sont l'accélération (\emph{Speedup}) et l'efficacité (\emph{Efficiency}), et sont définis comme suit:
\begin{equation}
